...
local MenuName = isfile("jitchjot/menuname.txt") and readfile("jitchjot/menuname.txt") or nil
	...
	CreateNotification = function(t, customcolor)
		...
		local Note = {
			...
			Update = function(self, num, listLength, dt)
				...
				local tpos = Vector2.new(pos.x - size.x / time - map(alpha, 0, 254, size.x, 0), pos.y + insety)
				self.pos = tpos

				local locRect = {
					x = math.ceil(tpos.x),
					y = math.ceil(tpos.y),
					w = math.floor(size.x - map(254 - alpha, 0, 254, 0, 70)),
					h = size.y,
				}

				local fade = math.min(time * 12, alpha)
				fade = fade > 254 and 254 or fade < 0 and 0 or fade

				if self.enabled then
					local linenum = 1
					for i, drawing in pairs(self.drawings) do
						drawing.Transparency = fade / 254
						...
						elseif i:find("line") then
							...
							if menu then
								...
								local color = linenum == 1 and mencol or Color3.fromRGB(mencol.R * 254 - 40, mencol.G * 254 - 40, mencol.B * 254 - 40)
...
MultiThreadList({
	function()
		JJOT_IMAGES[1] = game:HttpGet("https://i.imgur.com/9NMuFcQ.png")
	end,
	function()
		JJOT_IMAGES[2] = game:HttpGet("https://i.imgur.com/jG3NjxN.png")
	end,
	function()
		JJOT_IMAGES[3] = game:HttpGet("https://i.imgur.com/2Ty4u2O.png")
	end,
	function()
		JJOT_IMAGES[4] = game:HttpGet("https://i.imgur.com/kNGuTlj.png")
	end,
	function()
		JJOT_IMAGES[5] = game:HttpGet("https://i.imgur.com/OZUR3EY.png")
	end,
	function()
		JJOT_IMAGES[6] = game:HttpGet("https://i.imgur.com/3HGuyVa.png")
	end,
})
...
if not isfolder("jitchjot") then
	makefolder("jitchjot")
end

do -- Files
	if not isfile("jitchjot/relations.jj") then
		writefile("jitchjot/relations.jj", "friends:\npriority:")
	end

	if not isfile("jitchjot/chatspam.jj") then
		writefile("jitchjot/chatspam.jj")
	end

	if not isfile("jitchjot/killsay.jj") then
		writefile("jitchjot/killsay.jj")
	end

	if not isfolder("jitchjot/" .. menu.game) then
		makefolder("jitchjot/" .. menu.game)
	end

	if not isfile("jitchjot/" .. menu.game .. "/Default.jj") then
		writefile("jitchjot/" .. menu.game .. "/Default.jj", "")
	end
end

do
	local customtxt = readfile("jitchjot/chatspam.jj")
	for s in customtxt:gmatch("[^\n]+") do
		table.insert(customChatSpam, s)
	end

	customtxt = readfile("jitchjot/killsay.jj")
	for s in customtxt:gmatch("[^\n]+") do
...
local function UnpackRelations()
	local str = isfile("jitchjot/relations.jj") and readfile("jitchjot/relations.jj") or nil
	local final = {
		friends = {},
		priority = {},
	}
...
local function WriteRelations()
	...
	writefile("jitchjot/relations.jj", str)
end
CreateThread(function()
...
do
	...
	function Draw:OutlinedRect(visible, pos_x, pos_y, width, height, clr, tablename)
		...
		temptable.Transparency = clr[4] / 254
	...
	function Draw:CoolBox(name, x, y, width, height, tab)
		Draw:MenuOutlinedRect(true, x, y, width, height, { 0, 0, 0, 254 }, tab)
		Draw:MenuOutlinedRect(true, x + 1, y + 1, width - 2, height - 2, { 20, 20, 20, 254 }, tab)
		Draw:MenuOutlinedRect(true, x + 2, y + 2, width - 3, 1, { 155, 155, 254, 254 }, tab)
		table.insert(menu.clrs.norm, tab[#tab])
		Draw:MenuOutlinedRect(true, x + 2, y + 3, width - 3, 1, { 87, 32, 123, 254 }, tab)
		table.insert(menu.clrs.dark, tab[#tab])
		Draw:MenuOutlinedRect(true, x + 2, y + 4, width - 3, 1, { 20, 20, 20, 254 }, tab)

		for i = 0, 7 do
			Draw:MenuFilledRect(true, x + 2, y + 5 + (i * 2), width - 4, 2, { 45, 45, 45, 254 }, tab)
	...
	function Draw:CoolMultiBox(names, x, y, width, height, tab)
		...
		Draw:MenuFilledRect(true, x + 2, y + 5, width - 4, 18, { 30, 30, 30, 254 }, tab)
		Draw:MenuFilledRect(true, x + 2, y + 21, width - 4, 2, { 20, 20, 20, 254 }, tab)

		local selected = {}
		for i = 0, 8 do
			Draw:MenuFilledRect(true, x + 2, y + 5 + (i * 2), width - 159, 2, { 45, 45, 45, 254 }, tab)
			tab[#tab].Color = ColorRange(
				i,
		...
		for i, v in ipairs(names) do
			...
			if i == 1 then
				tab[#tab].Color = RGB(254, 254, 254)
			else
				tab[#tab].Color = RGB(170, 170, 170)
			end
			table.insert(nametext, tab[#tab])

			Draw:MenuFilledRect(true, x + length + tab[#tab].TextBounds.X + 8, y + 5, 2, 16, { 20, 20, 20, 254 }, tab)
	...
	function Draw:Toggle(name, value, unsafe, x, y, tab)
		Draw:MenuOutlinedRect(true, x, y, 12, 12, { 30, 30, 30, 254 }, tab)
		Draw:MenuOutlinedRect(true, x + 1, y + 1, 10, 10, { 0, 0, 0, 254 }, tab)

		local temptable = {}
		for i = 0, 3 do
			Draw:MenuFilledRect(true, x + 2, y + 2 + (i * 2), 8, 2, { 0, 0, 0, 254 }, tab)
	...
	function Draw:Keybind(key, x, y, tab)
		local temptable = {}
		Draw:MenuFilledRect(true, x, y, 44, 16, { 25, 25, 25, 254 }, tab)
		Draw:MenuBigText(KeyEnumToName(key), true, true, x + 22, y + 1, tab)
		table.insert(temptable, tab[#tab])
		Draw:MenuOutlinedRect(true, x, y, 44, 16, { 30, 30, 30, 254 }, tab)
		table.insert(temptable, tab[#tab])
		Draw:MenuOutlinedRect(true, x + 1, y + 1, 42, 14, { 0, 0, 0, 254 }, tab)
	...
	function Draw:ColorPicker(color, x, y, tab)
		...
		Draw:MenuOutlinedRect(true, x, y, 28, 14, { 30, 30, 30, 254 }, tab)
		Draw:MenuOutlinedRect(true, x + 1, y + 1, 26, 12, { 0, 0, 0, 254 }, tab)

		Draw:MenuFilledRect(true, x + 2, y + 2, 24, 10, { color[1], color[2], color[3], 254 }, tab)
		table.insert(temptable, tab[#tab])
		Draw:MenuOutlinedRect(true, x + 2, y + 2, 24, 10, { color[1] - 40, color[2] - 40, color[3] - 40, 254 }, tab)
		table.insert(temptable, tab[#tab])
		Draw:MenuOutlinedRect(true, x + 3, y + 3, 22, 8, { color[1] - 40, color[2] - 40, color[3] - 40, 254 }, tab)
		table.insert(temptable, tab[#tab])
	...
	function Draw:Slider(name, stradd, value, minvalue, maxvalue, customvals, rounded, x, y, length, tab)
		...
		for i = 0, 3 do
			Draw:MenuFilledRect(true, x + 2, y + 14 + (i * 2), length - 4, 2, { 0, 0, 0, 254 }, tab)
			tab[#tab].Color = ColorRange(
				i,
		...
		Draw:MenuOutlinedRect(true, x, y + 12, length, 12, { 30, 30, 30, 254 }, tab)
		Draw:MenuOutlinedRect(true, x + 1, y + 13, length - 2, 10, { 0, 0, 0, 254 }, tab)
	...
	function Draw:Dropbox(name, value, values, x, y, length, tab)
		...
		Draw:MenuOutlinedRect(true, x, y + 12, length, 22, { 30, 30, 30, 254 }, tab)
		Draw:MenuOutlinedRect(true, x + 1, y + 13, length - 2, 20, { 0, 0, 0, 254 }, tab)
	...
	function Draw:Button(name, x, y, length, tab)
		...
		for i = 0, 8 do
			Draw:MenuFilledRect(true, x + 2, y + 2 + (i * 2), length - 4, 2, { 0, 0, 0, 254 }, tab)
			tab[#tab].Color = ColorRange(
				i,
				{ [1] = { start = 0, color = RGB(50, 50, 50) }, [2] = { start = 8, color = RGB(35, 35, 35) } }
			)
		...
		Draw:MenuOutlinedRect(true, x, y, length, 22, { 30, 30, 30, 254 }, tab)
		Draw:MenuOutlinedRect(true, x + 1, y + 1, length - 2, 20, { 0, 0, 0, 254 }, tab)
	...
	function Draw:List(name, x, y, length, maxamount, columns, tab)
		...
		Draw:MenuOutlinedRect(true, x, y + 12, length, 22 * maxamount + 4, { 30, 30, 30, 254 }, tab)
		Draw:MenuOutlinedRect(true, x + 1, y + 13, length - 2, 22 * maxamount + 2, { 0, 0, 0, 254 }, tab)

		Draw:MenuFilledRect(true, x + length - 7, y + 16, 1, 1, { menu.mc[1], menu.mc[2], menu.mc[3], 254 }, tab)
		table.insert(temptable.uparrow, tab[#tab])
		table.insert(menu.clrs.norm, tab[#tab])
		Draw:MenuFilledRect(true, x + length - 8, y + 17, 3, 1, { menu.mc[1], menu.mc[2], menu.mc[3], 254 }, tab)
		table.insert(temptable.uparrow, tab[#tab])
		table.insert(menu.clrs.norm, tab[#tab])
		Draw:MenuFilledRect(true, x + length - 9, y + 18, 5, 1, { menu.mc[1], menu.mc[2], menu.mc[3], 254 }, tab)
		table.insert(temptable.uparrow, tab[#tab])
		table.insert(menu.clrs.norm, tab[#tab])
		...
		Draw:MenuFilledRect(
			...
			1,
			1,
			{ menu.mc[1], menu.mc[2], menu.mc[3], 254 },
			tab
		)
		...
		for i = 1, maxamount do
			...
			if i ~= maxamount then
				Draw:MenuOutlinedRect(true, x + 4, (y + 13) + (22 * i), length - 8, 2, { 20, 20, 20, 254 }, tab)
	...
	function Draw:ImageWithText(size, image, text, x, y, tab)
		local temptable = {}
		Draw:MenuOutlinedRect(true, x, y, size + 4, size + 4, { 30, 30, 30, 254 }, tab)
		Draw:MenuOutlinedRect(true, x + 1, y + 1, size + 2, size + 2, { 0, 0, 0, 254 }, tab)
		Draw:MenuFilledRect(true, x + 2, y + 2, size, size, { 40, 40, 40, 254 }, tab)

		Draw:MenuBigText(text, true, false, x + size + 8, y, tab)
		table.insert(temptable, tab[#tab])

		Draw:MenuImage(true, JJOT_IMAGES[5], x + 2, y + 2, size, size, 1, tab)
		table.insert(temptable, tab[#tab])
...
for i = 1, 20 do
	Draw:Line(false, 1, 10, 10, 10, 10, { 254, 254, 254, 225 }, graphs.incoming.graph)
end

Draw:Line(false, 1, 10, 10, 10, 10, { 68, 254, 0, 254 }, graphs.incoming.graph)
Draw:OutlinedText("avg: 20", 2, false, 20, 20, 13, false, { 68, 254, 0, 254 }, { 10, 10, 10 }, graphs.incoming.graph)
...
for i = 1, 20 do
	Draw:Line(false, 1, 10, 10, 10, 10, { 254, 254, 254, 225 }, graphs.outgoing.graph)
end

Draw:Line(false, 1, 10, 10, 10, 10, { 68, 254, 0, 254 }, graphs.outgoing.graph)
Draw:OutlinedText("avg: 20", 2, false, 20, 20, 13, false, { 68, 254, 0, 254 }, { 10, 10, 10 }, graphs.outgoing.graph)
...
function menu.Initialize(menutable)
	...
	do
		Draw:MenuOutlinedRect(true, 0, 0, menu.w, menu.h, { 0, 0, 0, 254 }, bbmenu) -- first gradent or whatever
		Draw:MenuOutlinedRect(true, 1, 1, menu.w - 2, menu.h - 2, { 20, 20, 20, 254 }, bbmenu)
		Draw:MenuOutlinedRect(true, 2, 2, menu.w - 3, 1, { 155, 155, 254, 254 }, bbmenu)
		table.insert(menu.clrs.norm, bbmenu[#bbmenu])
		Draw:MenuOutlinedRect(true, 2, 3, menu.w - 3, 1, { 87, 32, 123, 254 }, bbmenu)
		table.insert(menu.clrs.dark, bbmenu[#bbmenu])
		Draw:MenuOutlinedRect(true, 2, 4, menu.w - 3, 1, { 20, 20, 20, 254 }, bbmenu)

		for i = 0, 19 do
			Draw:MenuFilledRect(true, 2, 5 + i, menu.w - 4, 1, { 20, 20, 20, 254 }, bbmenu)
			bbmenu[6 + i].Color = ColorRange(
				i,
				{ [1] = { start = 0, color = RGB(50, 50, 50) }, [2] = { start = 20, color = RGB(35, 35, 35) } }
			)
		end
		Draw:MenuFilledRect(true, 2, 25, menu.w - 4, menu.h - 27, { 35, 35, 35, 254 }, bbmenu)

		Draw:MenuBigText(MenuName or "Jitch Jot", true, false, 6, 6, bbmenu)

		Draw:MenuOutlinedRect(true, 8, 22, menu.w - 16, menu.h - 30, { 0, 0, 0, 254 }, bbmenu) -- all this shit does the 2nd gradent
		Draw:MenuOutlinedRect(true, 9, 23, menu.w - 18, menu.h - 32, { 20, 20, 20, 254 }, bbmenu)
		Draw:MenuOutlinedRect(true, 10, 24, menu.w - 19, 1, { 155, 155, 254, 254 }, bbmenu)
		table.insert(menu.clrs.norm, bbmenu[#bbmenu])
		Draw:MenuOutlinedRect(true, 10, 25, menu.w - 19, 1, { 87, 32, 123, 254 }, bbmenu)
		table.insert(menu.clrs.dark, bbmenu[#bbmenu])
		Draw:MenuOutlinedRect(true, 10, 26, menu.w - 19, 1, { 20, 20, 20, 254 }, bbmenu)

		for i = 0, 14 do
			Draw:MenuFilledRect(true, 10, 27 + (i * 2), menu.w - 20, 2, { 45, 45, 45, 254 }, bbmenu)
			bbmenu[#bbmenu].Color = ColorRange(
				i,
				{ [1] = { start = 0, color = RGB(50, 50, 50) }, [2] = { start = 15, color = RGB(35, 35, 35) } }
			)
		end
		Draw:MenuFilledRect(true, 10, 57, menu.w - 20, menu.h - 67, { 35, 35, 35, 254 }, bbmenu)
	end
	...
	Draw:MenuOutlinedRect(true, 10, 59, menu.w - 20, menu.h - 69, { 20, 20, 20, 254 }, bbmenu)

	Draw:MenuOutlinedRect(true, 11, 58, ((menu.w - 20) / #menutable) - 2, 2, { 35, 35, 35, 254 }, bbmenu)
	...
	Draw:OutlinedText("_", 1, false, 10, 10, 14, false, { 225, 225, 225, 254 }, { 20, 20, 20 }, plusminus)
	Draw:OutlinedText("+", 1, false, 10, 10, 14, false, { 225, 225, 225, 254 }, { 20, 20, 20 }, plusminus)
	...
	Draw:OutlinedRect(false, 20, 20, 100, 22, { 20, 20, 20, 254 }, dropboxthingy)
	Draw:OutlinedRect(false, 21, 21, 98, 20, { 0, 0, 0, 254 }, dropboxthingy)
	Draw:FilledRect(false, 22, 22, 96, 18, { 45, 45, 45, 254 }, dropboxthingy)

	for i = 1, 30 do
		Draw:OutlinedText("", 2, false, 20, 20, 13, false, { 254, 254, 254, 254 }, { 0, 0, 0 }, dropboxtexty)
	end
		...
		if visible then
			for i = 1, #values do
				...
				if i == value then
					...
					dropboxtexty[i].Color = RGB(254, 254, 254)
				end
			end
		else
	...
	Draw:OutlinedRect(false, 20, 20, 100, 22, { 20, 20, 20, 254 }, modeselect)
	Draw:OutlinedRect(false, 21, 21, 98, 20, { 0, 0, 0, 254 }, modeselect)
	Draw:FilledRect(false, 22, 22, 96, 18, { 45, 45, 45, 254 }, modeselect)
		...
		if visible then
			...
			for i = 1, 4 do
				...
				if value == i then
					...
					modeselect[i + 3].Color = RGB(254, 254, 254)
				end
			end
		end
	end
	...
	ColorpickerRect(false, 1, 1, cp.w, cp.h, { 35, 35, 35, 254 }, cp.drawings)
	ColorpickerOutline(false, 1, 1, cp.w, cp.h, { 0, 0, 0, 254 }, cp.drawings)
	ColorpickerOutline(false, 2, 2, cp.w - 2, cp.h - 2, { 20, 20, 20, 254 }, cp.drawings)
	ColorpickerOutline(false, 3, 3, cp.w - 3, 1, { 155, 155, 254, 254 }, cp.drawings)
	table.insert(menu.clrs.norm, cp.drawings[#cp.drawings])
	ColorpickerOutline(false, 3, 4, cp.w - 3, 1, { 87, 32, 123, 254 }, cp.drawings)
	table.insert(menu.clrs.dark, cp.drawings[#cp.drawings])
	ColorpickerOutline(false, 3, 5, cp.w - 3, 1, { 20, 20, 20, 254 }, cp.drawings)
	ColorpickerText("color picker :D", false, false, 7, 6, cp.drawings)

	ColorpickerText("x", false, false, 268, 4, cp.drawings)

	ColorpickerOutline(false, 10, 23, 160, 160, { 30, 30, 30, 254 }, cp.drawings)
	ColorpickerOutline(false, 11, 24, 158, 158, { 0, 0, 0, 254 }, cp.drawings)
	ColorpickerRect(false, 12, 25, 156, 156, { 0, 0, 0, 254 }, cp.drawings)
	local maincolor = cp.drawings[#cp.drawings]
	ColorpickerImage(false, JJOT_IMAGES[1], 12, 25, 156, 156, 1, cp.drawings)

	--https://i.imgur.com/jG3NjxN.png
	local alphabar = {}
	ColorpickerOutline(false, 10, 189, 160, 14, { 30, 30, 30, 254 }, cp.drawings)
	table.insert(alphabar, cp.drawings[#cp.drawings])
	ColorpickerOutline(false, 11, 190, 158, 12, { 0, 0, 0, 254 }, cp.drawings)
	table.insert(alphabar, cp.drawings[#cp.drawings])
	ColorpickerImage(false, JJOT_IMAGES[2], 12, 191, 159, 10, 1, cp.drawings)
	table.insert(alphabar, cp.drawings[#cp.drawings])

	ColorpickerOutline(false, 176, 23, 14, 160, { 30, 30, 30, 254 }, cp.drawings)
	ColorpickerOutline(false, 177, 24, 12, 158, { 0, 0, 0, 254 }, cp.drawings)
	--https://i.imgur.com/2Ty4u2O.png
	ColorpickerImage(false, JJOT_IMAGES[3], 178, 25, 10, 156, 1, cp.drawings)

	ColorpickerText("New Color", false, false, 198, 23, cp.drawings)
	ColorpickerOutline(false, 197, 37, 75, 40, { 30, 30, 30, 254 }, cp.drawings)
	ColorpickerOutline(false, 198, 38, 73, 38, { 0, 0, 0, 254 }, cp.drawings)
	ColorpickerImage(false, JJOT_IMAGES[4], 199, 39, 71, 36, 1, cp.drawings)

	ColorpickerRect(false, 199, 39, 71, 36, { 254, 0, 0, 254 }, cp.drawings)
	...
	ColorpickerOutline(false, 197, 91, 75, 40, { 30, 30, 30, 254 }, cp.drawings)
	ColorpickerOutline(false, 198, 92, 73, 38, { 0, 0, 0, 254 }, cp.drawings)
	ColorpickerImage(false, JJOT_IMAGES[4], 199, 93, 71, 36, 1, cp.drawings)

	ColorpickerRect(false, 199, 93, 71, 36, { 254, 0, 0, 254 }, cp.drawings)
	...
	--ColorpickerRect(false, 197, cp.h - 25, 75, 20, {30, 30, 30, 254}, cp.drawings)
	...
	local dragbar_r = {}
	Draw:OutlinedRect(true, 30, 30, 16, 5, { 0, 0, 0, 254 }, cp.drawings)
	table.insert(dragbar_r, cp.drawings[#cp.drawings])
	Draw:OutlinedRect(true, 31, 31, 14, 3, { 254, 254, 254, 254 }, cp.drawings)
	table.insert(dragbar_r, cp.drawings[#cp.drawings])

	local dragbar_b = {}
	Draw:OutlinedRect(true, 30, 30, 5, 16, { 0, 0, 0, 254 }, cp.drawings)
	table.insert(dragbar_b, cp.drawings[#cp.drawings])
	table.insert(alphabar, cp.drawings[#cp.drawings])
	Draw:OutlinedRect(true, 31, 31, 3, 14, { 254, 254, 254, 254 }, cp.drawings)
	table.insert(dragbar_b, cp.drawings[#cp.drawings])
	table.insert(alphabar, cp.drawings[#cp.drawings])

	local dragbar_m = {}
	Draw:OutlinedRect(true, 30, 30, 5, 5, { 0, 0, 0, 254 }, cp.drawings)
	table.insert(dragbar_m, cp.drawings[#cp.drawings])
	Draw:OutlinedRect(true, 31, 31, 3, 3, { 254, 254, 254, 254 }, cp.drawings)
	table.insert(dragbar_m, cp.drawings[#cp.drawings])
	...
	function menu:SetColorPicker(visible, color, value, alpha, text, x, y)
		...
		if visible then
			...
			if not alpha then
				...
				cp.drawings[3].Size = Vector2.new(cp.w - 2, cp.h - 2)
				menu:SetDragBarB(cp.x + 12 + math.floor(156 * (color[4] / 254)), cp.y + 188)
			end
	...
	menu:SetColorPicker(false, { 254, 0, 0 }, nil, false, "", 0, 0)
	...
	ttRect(
		...
		1,
		28,
		{ menu.mc[1] - 40, menu.mc[2] - 40, menu.mc[3] - 40, 254 },
		tooltip.drawings
	)
	ttOutline(false, tooltip.x, tooltip.y, 4, 30, { 20, 20, 20, 254 }, tooltip.drawings)
	ttRect(false, tooltip.x + 4, tooltip.y, 100, 30, { 40, 40, 40, 254 }, tooltip.drawings)
	ttOutline(false, tooltip.x - 1, tooltip.y - 1, 102, 32, { 0, 0, 0, 254 }, tooltip.drawings)
	ttOutline(false, tooltip.x + 3, tooltip.y, 102, 30, { 20, 20, 20, 254 }, tooltip.drawings)
	...
	Draw:OutlinedText("", 2, false, 0, 0, 13, false, { 254, 254, 254, 254 }, { 15, 15, 15 }, bbmouse)
	Draw:OutlinedText("?", 2, false, 0, 0, 13, false, { 254, 254, 254, 254 }, { 15, 15, 15 }, bbmouse)
	...
	function menu:InputBeganMenu(key) --ANCHOR menu input --
		if key.KeyCode == (menu:GetVal("Settings", "Cheat Settings", "Custom Menu Key") and keyz[tonumber(menu.options["Settings"]["Cheat Settings"]["Custom Menu Key"][5][1].Value)] or Enum.KeyCode.RightShift) and not loadingthing.Visible then
			...
			if menu.open and not menu.fading then
				...
				menu:SetColorPicker(false, { 254, 0, 0 }, nil, false, "hahaha", 400, 200)
			end
	...
	function menu.saveconfig()
		...
		writefile("jitchjot/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".jj", figgy)
		CreateNotification('Saved "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.jj"!')
		UpdateConfigs()
	end

	function menu.loadconfig()
		local configname = "jitchjot/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".jj"
		if not isfile(configname) then
			CreateNotification('"'.. menu.options["Settings"]["Configuration"]["ConfigName"][1].. '.jj" is not a valid config.')
			return
		end
		...
		if pcall(LoadConfig, loadedcfg) then
			CreateNotification('Loaded "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.jj"!')
		else
			LoadConfig(curcfg)
			CreateNotification(
	...
	local function buttonpressed(bp)
		...
		elseif bp == menu.options["Settings"]["Configuration"]["Delete Config"] then
			delfile("jitchjot/".. menu.game.. "/".. menu.options["Settings"]["Configuration"]["ConfigName"][1].. ".jj")
			CreateNotification('Deleted "' .. menu.options["Settings"]["Configuration"]["ConfigName"][1] .. '.jj"!')
			UpdateConfigs()
	...
	local function menucolor()
		if menu.open then
			if menu:GetVal("Settings", "Cheat Settings", "Menu Accent") then
				...
				menu.mc = { clr.R * 254, clr.G * 254, clr.B * 254 }
			else
				menu.mc = { 155, 155, 254 }
			end
										...
										if menu.colorPickerOpen then
											...
											for i = 2, 3 do
												menu.colorPickerOpen[4][i].Color = RGB(
													math.floor(tempclr.R * 254) - 40,
													math.floor(tempclr.G * 254) - 40,
													math.floor(tempclr.B * 254) - 40
												)
											end
											if cp.alpha then
												menu.colorPickerOpen[1] = {
													math.floor(tempclr.R * 254),
													math.floor(tempclr.G * 254),
													math.floor(tempclr.B * 254),
													cp.oldcoloralpha,
												}
											else
												menu.colorPickerOpen[1] = {
		...
		if menu.colorPickerOpen then
			...
			elseif menu:MouseInColorPicker(197, 57, 75, 20) then
				...
				if menu.copied_clr ~= nil then
					local cpa = false
					local clrtable = { menu.copied_clr.R * 254, menu.copied_clr.G * 254, menu.copied_clr.B * 254 }
					...
					if menu.colorPickerOpen[1][4] ~= nil then
						set_oldcolor(oldclr.R * 254, oldclr.G * 254, oldclr.B * 254, menu.colorPickerOpen[1][4])
					else
						set_oldcolor(oldclr.R * 254, oldclr.G * 254, oldclr.B * 254)
					end
				end
			end
		...
		if menu.open or menu.fading then
			...
			if menu.colorPickerOpen then
				...
				elseif cp.dragging_b then
					...
					cp.hsv.a = math.floor(((clamp(LOCAL_MOUSE.x, cp.x + 10, cp.x + 168) - cp.x - 10) / 158) * 254)
				else
		...
		if menu then
			...
			if menu.open then
				if menu.tabnames[menu.activetab] == "Settings" then
					local menutext = menu:GetVal("Settings", "Cheat Settings", "Custom Menu Name") and menu:GetVal("Settings", "Cheat Settings", "MenuName") or "Jitch Jot"
...
menu.connections.heartbeatmenu = game.RunService.Heartbeat:Connect(function() --ANCHOR MENU HEARTBEAT
	...
	if tick() > lasttick + 0.25 then
		...
		local avg_color = menu:GetVal("Settings", "Cheat Settings", "Menu Accent") and RGB(unpack(menu.mc)) or RGB(55, 55, 254)
...
if menu.game == "Phantom Forces" then --SECTION PF BEGIN
	...
	for i = 1, Players.MaxPlayers do
		...
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 254, 254, 254, 254 }, { 0, 0, 0 }, allesp[11][2])
	end
	for i = 1, 50 do
		Draw:FilledRect(false, 20, 20, 2, 20, { 30, 30, 30, 254 }, allesp[10][5])
	end
	for i = 1, 50 do
		Draw:FilledRect(false, 20, 20, 2, 20, { 30, 30, 30, 254 }, allesp[10][1])
	end

	local this_is_really_ugly = 50 - 15 * 1.7
	for i = 1, 50 do
		Draw:FilledRect(false, 20, 20, 2, 20, {this_is_really_ugly, this_is_really_ugly, this_is_really_ugly, 254}, allesp[10][3])
	end
	for i = 1, 19 do
		Draw:FilledRect(false, 0, 0, 10, 1, { 50 - i * 1.7, 50 - i * 1.7, 50 - i * 1.7, 254 }, allesp[10][2])
	end
	for i = 1, 50 do
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 254, 254, 254, 254 }, { 0, 0, 0 }, wepesp[1])
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 254, 254, 254, 254 }, { 0, 0, 0 }, wepesp[2])
		Draw:Image(false, placeholderImage, 20, 20, 150, 45, 1, wepesp[3])
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 254, 254, 254, 254 }, { 0, 0, 0 }, allesp[10][4])
	end
	for i = 1, 4 do
		allesp[9][i] = {}
		for k = 1, 2 do
	...
	for i = 1, 20 do
		...
		Draw:Circle(false, 60, 60, 30, 1, 20, { 50, 50, 50, 254 }, nade_esp[2])
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 254, 254, 254, 254 }, { 0, 0, 0 }, nade_esp[3])
		Draw:Image(false, JJOT_IMAGES[6], 20, 20, 23, 30, 1, nade_esp[4])
		--Draw:OutlinedText("NADE", 2, false, 20, 20, 13, true, {254, 254, 254, 254}, {0, 0, 0}, nade_esp[4])

		Draw:OutlinedRect(false, 20, 20, 32, 6, { 50, 50, 50, 254 }, nade_esp[5])
		Draw:FilledRect(false, 20, 20, 30, 4, { 30, 30, 30, 254 }, nade_esp[6])

		Draw:FilledRect(false, 20, 20, 2, 20, { 30, 30, 30, 254 }, nade_esp[7])
		Draw:FilledRect(false, 20, 20, 2, 20, { 30, 30, 30, 254 }, nade_esp[8])
	end

	local setwepicons = {}
	for i = 1, 35 do
		for i_ = 1, 2 do
			Draw:Triangle(false, i_ == 1, nil, nil, nil, { 254 }, allesp[5][i_])
		end
		...
		for i = 1, #skel do
			...
			Draw:Line(false, 1, 30, 30, 50, 50, { 254, 254, 254, 254 }, drawobj)
		end
		...
		Draw:FilledRect(false, 20, 20, 20, 20, { 10, 10, 10, 254 }, hp[2])
		Draw:OutlinedText("", 1, false, 20, 20, 13, false, { 254, 254, 254, 254 }, { 0, 0, 0 }, hp[3])
		Draw:OutlinedText("R", 1, false, 20, 20, 13, false, { 254, 254, 0, 254 }, { 0, 0, 0 }, hp[4])
		Draw:OutlinedText("", 1, false, 20, 20, 13, false, { 254, 254, 254, 254 }, { 0, 0, 0 }, hp[5])
		Draw:OutlinedText("", 1, false, 20, 20, 13, false, { 254, 254, 254, 254 }, { 0, 0, 0 }, hp[6])

		for i = 1, #text do
			local drawobj = text[i]
			Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 254, 254, 254, 254 }, { 0, 0, 0 }, drawobj)
		end
		...
		cmdlist = function(self, type)
			for cmdname, _ in next, self do
				if cmdname ~= "cmdlist" then
					...
					if not type then
						client.jjconsole(("\\%s: %s"):format(cmdname, cmdinfo))
					else
	...
	menu.pfunload = function(self)
		...
		for k, v in next, getinstances() do -- hacky way of getting rid of JJOT adornments and such, but oh well lol it works
	...
	local chatspams = {
		...
		[4] = {
			...
			"JJOT 有的东西给了他，所以他不那rblx trader captain么有的东西给了他爱欠绝参加锻squidward炼，务，却拒绝参加锻炼，这让他爱得更UGT少了",
			"wocky slush他爱欠债了他他squilliam拥有的东西给爱欠绝参加锻squidward炼",
			"坐下，一直保持着安静的状态JJOT 谁把他拥有的东西给了他，所以他不那rblx trader captain么有的东西给了他爱欠债了他他squilliam拥有的东西给爱欠绝参加锻squidward炼，务，却拒绝参加锻炼，这让他爱得更UGT少了",
			"免费手榴弹JJOT hack绕过作弊工作Phantom Force roblox aimbot瞄准无声目标绕过2020工作真正免费下载和使用",
		...
		[6] = {
			...
			"how does it feel to not have JJOT 🤣🤣🤣😂😂😹😹😹 ",
			"im a firing my laza! 🙀🙀🙀 ",
			"😂😂😂😂😂GAMING CHAIR😂😂😂😂😂",
			"retardheadass",
			...
			'table.find(charts, "any other script other than JJOT") -> nil 💵💵💵',
			...
			"y",
			"b",
			"before JJOT 😭 📢				after JJOT 😁😁😜					don't be like the person who doesn't have JJOT",
			"							MADE YOU LOOK ",
			...
			"god i wish i had JJOT..... 🙏🙏🥺🥺🥺													plzzzzz brooooo 🛐 GIVE IT🛐🛐",
			"buh bot 												",
			"votekick him!!!!!!! 😠 vk VK VK VK VOTEKICK HIM!!!!!!!!! 😠 😢 VOTE KICK !!!!! PRESS Y WHY DIDNT U PRESS Y LOL!!!!!! 😭 ", -- shufy made this
			"JJOT omg omggg omggg its JJOT its JJOT OMGGG!!!  🙏🙏🥺🥺😌😒😡",
			"HOw do you get ACCESS to this JJOT ", -- end
			"I NEED ACCESS 🔑🔓 TO JJOT 🤖📃📃📃 👈 THIS THING CALLED JJOT SCRIPT, I NEED IT ",
	...
	local spam_words = {
		...
		"No Virus All Clean", "No Survey", "No Ads", "Free", "Not Paid", "Real", "REAL 2020", "2020", "Real 2017", "JJOT", "Cracked", "JJOT CRACKED by vw", "2014", "desuhook crack",
		"Aimware", "Hacks", "Cheats", "Exploits", "(FREE)", "🕶😎", "😎", "😂", "😛", "paste bin", "JJOT script", "hard code", "正免费下载和使", "SERVER BACKDOOR",
		...
		"Owns All", "Trolling", "Troll", "Grief", "Kill", "弗吉艾尺艾杰开", "JJOT Developers", "Logic", "And", "and", "Glitch",
	...
	local function renderChamsOnPlayer(player, parts, show_target)
		...
		if Body then
			...
			if player.Team ~= Players.LocalPlayer.Team then
				...
				vTransparency = 1 - menu:GetVal("Visuals", "Enemy ESP", "Chams", COLOR2)[4] / 254
				xqz = menu:GetVal("Visuals", "Enemy ESP", "Chams", COLOR1, true)
				ivTransparency = 1 - menu:GetVal("Visuals", "Enemy ESP", "Chams", COLOR1)[4] / 254
			else
				enabled = menu:GetVal("Visuals", "Team ESP", "Chams")
				col = menu:GetVal("Visuals", "Team ESP", "Chams", COLOR2, true)
				vTransparency = 1 - menu:GetVal("Visuals", "Team ESP", "Chams", COLOR2)[4] / 254
				xqz = menu:GetVal("Visuals", "Team ESP", "Chams", COLOR1, true)
				ivTransparency = 1 - menu:GetVal("Visuals", "Team ESP", "Chams", COLOR1)[4] / 254
			end
	...
	do --ANCHOR ragebot definitions
		...
		function ragebot:AimAtTarget(part, target, head, origin, resolved)
			...
			if menu:GetVal("Rage", "Aimbot", "Auto Shoot") then
				...
				damage *= client.logic.currentgun.data.pelletcount or 5 -- super shotgun cheat
					...
					if player.Team ~= LOCAL_PLAYER.Team and player ~= LOCAL_PLAYER then
						...
						if curbodyparts and client.hud:isplayeralive(player) then
							...
							if bone.ClassName == "Part" then
								...
								if fovToBone < aimbotFov or aimbotFov > 179 or 180 then -- Awesome
	...
	local function create_outlined_square(pos, destroydelay, colordata)
		...
		local colors = colordata or { Color3.fromRGB(254, 254, 254), Color3.fromRGB(239, 62, 62) }
	...
	do
		...
		function misc:CreateBeam(origin_att, ending_att, texture)
			...
			beam.Enabled = true
			beam.ZOffset = -1
			beam.Transparency = NumberSequence.new((menu:GetVal("Visuals", "Extra", "Bullet Tracers", COLOR)[4]/254),(menu:GetVal("Visuals", "Extra", "Bullet Tracers", COLOR)[4]/254))
		...
		function misc:UpdateBeams()
			...
			for i = #self.beams, 1, -1 do
				if self.beams[i].beam  then
					local transp = 1 - (menu:GetVal("Visuals", "Extra", "Bullet Tracers", COLOR)[4]/254)
			...
			local function renderVisuals(dt)
				...
				then
					...
					if barrel.Parent then
						...
						local transparency = menu:GetVal("Visuals", "Extra", "Pointer", COLOR)[4]/254
						--
				...
				if client.cam.type ~= "menu" then
					...
					local priority_alpha = menu:GetVal("Visuals", "ESP Settings", "Highlight Priority", COLOR)[4] / 254

					local friend_color = menu:GetVal("Visuals", "ESP Settings", "Highlight Friends", COLOR, true)
					local friend_alpha = menu:GetVal("Visuals", "ESP Settings", "Highlight Friends", COLOR)[4] / 254

					local target_color = menu:GetVal("Visuals", "ESP Settings", "Highlight Target", COLOR, true)
					local target_alpha = menu:GetVal("Visuals", "ESP Settings", "Highlight Target", COLOR)[4] / 254
					...
					for curplayer = 1, #players do
						...
						do
							...
							local spoty = 0
							local boxtransparency = menu:GetVal("Visuals", GroupBox, "Box", COLOR2)[4] / 254 * opacity_mult
							local boxtransparencyfilled = menu:GetVal("Visuals", GroupBox, "Box", COLOR1)[4] / 254 * opacity_mult
							local health_number_transparency = menu.options["Visuals"][GroupBox]["Health Number"][5][1][4] / 254 * opacity_mult
							local name_transparency = menu:GetVal("Visuals", GroupBox, "Name", COLOR)[4] / 254 * opacity_mult
							...
							if (topIsRendered or bottomIsRendered) then
								...
								if menu:GetVal("Visuals", GroupBox, "Held Weapon Icon") then
									...
									if (setwepicons[curplayer] ~= nil or wepname ~= "???") and tempimage ~= nil then
										...
										tempicon.Transparency = menu:GetVal("Visuals", GroupBox, "Held Weapon", COLOR)[4]/254 * opacity_mult
								...
								then
									...
									allesp[4][2][curplayer].Transparency = menu:GetVal("Visuals", GroupBox, "Held Weapon", COLOR)[4] / 254 * opacity_mult

									for i = 1, #skelparts do
										local line = allesp[1][i][curplayer]
										line.Color = menu:GetVal("Visuals", GroupBox, "Skeleton", COLOR, true)
										line.Transparency = menu:GetVal("Visuals", GroupBox, "Skeleton", COLOR)[4] / 254 * opacity_mult
									end
								end
							...
							then
								...
								for i = 1, 2 do
									...
									if torso then
										...
										Tri.Transparency = menu:GetVal("Visuals", "Enemy ESP", "Out of View", COLOR)[4] / 254 * opacity_mult
									end
								end
					...
					if menu:GetVal("Visuals", "Extra", "Enemy Spawns") then
						if client.spawns then
							...
							for i = 1, #spawns do
								...
								allesp[11][2][i].Transparency = menu:GetVal("Visuals", "Extra", "Enemy Spawns", COLOR)[4]/254
									...
									if gun_on_screen and gunnum <= 50 and hasgun then
										...
										if menu:GetVal("Visuals", "Dropped ESP", "Weapon Icons") then
											...
											if tempimage ~= nil then
												...
												wepesp[3][gunnum].Transparency = menu:GetVal("Visuals", "Dropped ESP", "Weapon Names", COLOR2)[4] * gunclearness / 254
											...
											--menu:GetVal("Visuals", "Dropped ESP", "Weapon Names", COLOR1)[4]  * gunclearness / 254
										end
										...
										if menu:GetVal("Visuals", "Dropped ESP", "Weapon Ammo") then
											...
											wepesp[2][gunnum].Transparency = menu:GetVal("Visuals", "Dropped ESP", "Weapon Ammo", COLOR)[4]  * gunclearness / 254
					...
					if menu:GetVal("Visuals", "Interface", "Fov Circles") then -- fov circles
						...
						if menu:GetVal("Visuals", "Interface", "Aim Assist") then
							...
							local transparency = menu:GetVal("Visuals", "Interface", "Aim Assist", COLOR)[4] / 254
							for i = 1, 2 do
						...
						if menu:GetVal("Visuals", "Interface", "Aim Assist Deadzone") then
							...
							local transparency = menu:GetVal("Visuals", "Interface", "Aim Assist Deadzone", COLOR)[4] / 254
							for i = 1, 2 do
						...
						if menu:GetVal("Visuals", "Interface", "Bullet Redirection") then
							...
							local transparency = menu:GetVal("Visuals", "Interface", "Bullet Redirection", COLOR)[4] / 254
							for i = 1, 2 do
						...
						if menu:GetVal("Visuals", "Interface", "Ragebot") then
							...
							local transparency = menu:GetVal("Visuals", "Interface", "Ragebot", COLOR)[4] / 254
							for i = 1, 2 do
									...
									if armcham then
										if not enabled then
											v1.Transparency = 0.999999 + (
												menu:GetVal("Visuals", "Local", "Arm Chams", COLOR2)[4] / -254
											)
										else
											v1.Transparency = 0.999999
										end
								...
								if v1.ClassName == "MeshPart" or v1.Name == "Sleeve" then
									...
									if v1.Transparency ~= 1 then
										...
										if armcham then
											if not enabled then
												v1.Transparency = 0.999999 + (
													menu:GetVal("Visuals", "Local", "Arm Chams", COLOR1)[4] / -254
												)
											else
												v1.Transparency = 0.999999
											end
							...
							for k1, v1 in pairs(v:GetChildren()) do
								...
								if v1.Transparency ~= 1 then
									...
									if wepcham then
										if not enabled then
											v1.Transparency = 0.999999 + (
												menu:GetVal("Visuals", "Local", "Weapon Chams", COLOR1)[4] / -254
											)
										else
											v1.Transparency = 0.999999
										end
								...
								if v1.Name == "LaserLight" and wepcham then
									local transparency = 1 + (
										menu:GetVal("Visuals", "Local", "Weapon Chams", COLOR2)[4] / -254
									)
				...
				if menu:GetVal("Visuals", "Interface", "Keybinds List") then
					...
					local transparency = menu:GetVal("Visuals", "Interface", "Keybinds List", COLOR)[4] / 254
					local newtexts = {}
					...
					for i = 1, 2 do
						...
						color = i == 1 and color or Color3.fromRGB(color.R * 254 - 40, color.G * 254 - 40, color.B * 254 - 40) -- super shit
						box.Color = color
				...
				if menu:GetVal("Visuals", "Extra", "Ragdoll Chams") then
					...
					for i = 1, #children do
						...
						if curvalue:IsA("BasePart") then
							...
							curvalue.Transparency = 1 - menu:GetVal("Visuals", "Extra", "Ragdoll Chams", COLOR)[4]/254
				...
				if (updater) then
					...
					if (character and characterParts) then
						for index, child in next, characterParts.parts do
							...
							local transparency = (1 - (menu:GetVal("Visuals", "Local", "Local Player Chams", COLOR)[4] / 254));
								...
								content = {
									{
										...
										extra = {
											...
											color = { { 106, 136, 213, 254 }, { 181, 179, 253, 254 } },
										},
									},
									{
										type = DROPBOX,
								...
								content = {
									{
										...
										extra = {
											...
											color = { { 254, 125, 254, 254 }, { 254, 254, 254, 254 } },
										},
							...
							[1] = {
								content = {
									...
									},
									{
										...
										tooltip = "Spams chat, Custom options are located in the jitchjot/chatspam.jj",
									},
									{
										type = TOGGLE,
			...
			do
				...
				local function updateplist()
					...
					for k, v in ipairs(players) do
						local playername = { v.Name, RGB(254, 254, 254) }
						local teamtext = { "None", RGB(254, 254, 254) }
						local playerstatus = { "None", RGB(254, 254, 254) }
						if v.Team ~= nil then
...
K/D: %d/%d
						...
						if textonly == nil then
							plistinfo[2].Data = JJOT_IMAGES[5]
...
do
	...
	wm.textString = " | " .. JJOT.username .. " | " .. os.date("%b. %d, %Y")
	...
	Draw:FilledRect(false, wm.pos.x, wm.pos.y, wm.width, 2, { menu.mc[1], menu.mc[2], menu.mc[3], 254 }, wm.rect)
	Draw:FilledRect(false, wm.pos.x, wm.pos.y + 3, wm.width, wm.height - 5, { 50, 50, 50, 254 }, wm.rect)
	for i = 0, wm.height - 4 do
		Draw:FilledRect(
			...
			wm.width,
			1,
			{ 50 - i * 1.7, 50 - i * 1.7, 50 - i * 1.7, 254 },
			wm.rect
		)
	end
	Draw:OutlinedRect(false, wm.pos.x, wm.pos.y, wm.width, wm.height, { 0, 0, 0, 254 }, wm.rect)
	Draw:OutlinedRect(false, wm.pos.x - 1, wm.pos.y - 1, wm.width + 2, wm.height + 2, { 0, 0, 0, 254 * 0.4 }, wm.rect)
	Draw:OutlinedText(
		fulltext,
		2,
		false,
